...
from steps.step_10.step_10_shap_workflow import run_shap_gridsearch
...
def do_shap(app, dataframes, features, run_id):
    all_feature_records = []
    ...
    for seed in SEEDS:
        ...
        for model_name, baseline_model in fitted_models.items():
            ...
            gs_df = run_shap_gridsearch(
                ...
                features=features, task=task, thresholds=SHAP_THRESHOLDS, seed=seed,
        ...
        for model_name, classifier, _, _ in CLASSIFIERS:
            basline_model = get_fitted_model(model_name, classifier, dataframes['X_Train'], dataframes['Y_Train'])
            gs_df = run_shap_gridsearch(
                app=app, base_folder=base_folder, dataframes=dataframes, y_bin=y_bin,
                features=features, task=task, thresholds=SHAP_THRESHOLDS, seed=seed,
                reporter_obj=reporter_obj, model_name=model_name,
