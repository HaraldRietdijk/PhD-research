...
def fallback_select_features(significance_df: pd.DataFrame, top_k: int = 5) -> List[str]:
    ...
    if "t-value" not in sdf.columns or "stat.significance" not in sdf.columns:
        return []
    candidates = get_positive_values()
    if candidates:
        return candidates
    candidates = get_absolute_value()
    if candidates:
        return candidates
    return get_smallest_p_value()

def train_model_via_classifier_selection(app, base_folder, dataframes, selected_features, num_of_classes, model_name):
...
# train_model_via_classifier_selection(app, base_folder, dataframes, selected_features, num_of_classes, model_name):
